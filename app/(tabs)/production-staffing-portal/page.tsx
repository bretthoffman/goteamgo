...
if (crewRes?.value) {
  ...
  const loadReqs = async () => { 
    try { 
      const r = await storage.get('sage:requests'); 
      if (r?.value) setRequests(JSON.parse(r.value).sort((a: any, b: any) => +new Date(b.requestedAt) - +new Date(a.requestedAt))); 
      else setRequests([]);
    } catch (error) { 
      setRequests([]); 
    } 
  ...
  const saveReqs = async (list: any[]) => { 
    try { 
      await storage.set('sage:requests', JSON.stringify(list)); 
      setRequests(list); 
    } catch (error) { 
      console.error('Save error:', error);
      throw error; 
  ...
  const reqPos = async (event: any, idx: number) => {
    ...
    try { await saveReqs([...requests, reqData]); alert('✅ Submitted!'); }
    catch { alert('❌ Failed'); }
  };
  ...
  const reject = async (req: any) => {
    try { await saveReqs(requests.map(r => r.id === req.id ? { ...r, status: 'rejected' } : r)); alert('✅ Rejected'); }
    catch { alert('❌ Failed'); }
  };
